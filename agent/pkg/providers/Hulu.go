...
func HuluJP(c core.HttpClient) core.Result {
	resp, err := core.GET(c, "https://id.hulu.jp/",
		core.H{"user-agent", "Mozilla/5.0 (Linux; Android 12; IPH-ON24 Build/HUAWEIIPH-ON24) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.105 Mobile Safari/537.36"},
	)
	if err != nil {
	...
	if strings.HasSuffix(resp.Header.Get("Location"), "restrict.html") {
...
func Hulu(c core.HttpClient) core.Result {
	...
	if res.Error.Name == "GEO_BLOCKED" {
		return core.Result{Status: core.StatusNo}
	}
	if res.Error.Name == "LOGIN_FORBIDDEN" {
		return core.Result{Status: core.StatusOK}
	}
	return core.Result{Status: core.StatusUnexpected}
}
